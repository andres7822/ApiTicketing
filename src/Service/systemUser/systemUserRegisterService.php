<?php
    ...
    class systemUserRegisterService{
         ...
         * @throws ORMException
         */
        public function create(?string $user, ?string $password, ?string $email, ?string $selfie, ?string $tag, ?string $fullName, ?string $address, ?string $phone, ?int $area, ?int $idSystemUserStatus, ?int $idSystemRole, ?int $tries, ?string $position, ?string $skype): systemUser{
            $systemUser = new systemUser($user, $password, $email, $selfie, $tag, $fullName, $address, $phone, $area, $idSystemUserStatus, $idSystemRole, $tries, $position, $skype);
